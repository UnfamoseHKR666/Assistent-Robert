# Ассистент "Роберт" Сырая версия 1.0 
# Для последущего улучшения работоспособности данного ассистента необходимо подключиться к глобальному серверу .
# Возможные проблемы на этапах разработки ассистента---- наличие голоса и работоспособности мозга программы .
...
# Их нужно сгенерировать под ваш голос на сайте SnowBoy/для процесса работы с ассистенном
model = 'Robert.pmdl'
model2 = 'stop.pmdl'
...
# Функция которая дает запрос к серверу Alina получает и говорит ответ
...
def mozg(f):
    ...
    global listen
    listen.emit([2])
    if (('открой' in f) and ('яндекс браузер' in f)):
    webbrowser.open('https://www.yandex.com')
    if (('быстро найди' in f) and ('приказываю найди' in f)):
...
def listencommand():
    ...
    try:
        ...
        print("Роберт не расслышал фразу")
        global dontlisten
        dontlisten.emit(['00'])
...
@thread
def hotworddetect():
    print('Для того чтобы дать голосовую команду сперва скажите Роберт, и подождите две секунды, а после произнесите команду')
    # Запускаем детектор ключевого слова "Роберт" на движке SnowBoy
...
def Say(t):
 ...
 class MyWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Запускаем детектор имени Роберт
        hotworddetect()
    ...
    # Сигнал для установки той или иной картинки в зависимости от состояния Роберта (слушаю / отдыхаю)
    ...
    # Ставит картинку отображаему. когда Роберт говорит
...
if__name__=="__main__" :
    app: object = QtWidgets.QApplication(sys.argv)
    myapp = MyWin()
    myapp.show()
